package io.github.treesitter.jtreesitter.witze;

import java.lang.foreign.*;

public final class TreeSitterWitze {
    ...
    private static final TreeSitterWitze INSTANCE = new TreeSitterWitze();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterWitze() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_witze");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-witze");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-witze");
